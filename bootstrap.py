...
def modify_angular_json():
    with open('frontend/angular.json', 'r') as file:
        data = json.load(file)
    project = 'khatt'.replace('_', '-')
    for lang in 'en:english,nl:dutch,ar:arabic'.split(','):
...
def activate_frontend():
    ...
    if framework == 'backbone':
        ...
        project_name = 'khatt'.replace('_', '-')
        Command(
            'Creating project',
            ['ng', 'new', project_name, '--prefix=kht',
                '--skipGit=true',
                '--skipInstall=true',
                '--style=scss']
        )()
        ...
        if '4200' != '4200':
            Command(
                'Set frontend port',
                ['ng', 'config', 'projects.khatt.architect.serve.options.port', '4200'],
                cwd="frontend"
            )()
    else:
